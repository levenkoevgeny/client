<template>
  ...
  >
    <base-list-layout-for-cadet-update
      ...
      title="Прохождение службы в МВД РБ"
    >
      ...
      <template v-slot:tbody>
        <tr
          v-for="mvdService in orderedMainList"
          :key="mvdService.id"
          @dblclick.stop="showUpdateMainItemModal(mvdService.id)"
        >
          <td>
            <div
              class="form-check d-flex align-items-center justify-content-center"
          ...
          <td>
            <div class="d-flex align-items-end justify-content-end">
              <button
                type="button"
                class="btn btn-outline-danger"
                @click="trashButtonClick(mvdService.id)"
...
import MVDHistoryModalForCadetUpdate from "@/components/cadet/service/modals/MVDHistoryModalForCadetUpdate.vue"
...
export default {
  ...
  data() {
    return {
      ...
      mainItemAPIInstance: getMVDHistoryAPIInstance(),
      itemForm: Object.assign({}, getMVDHistoryAPIInstance().formData),
      selectedItems: [],
      deleteItemId: "",
    }
  },
  ...
  methods: {
    async loadData() {
      ...
      try {
        ...
        this.mainItemList = mvdHistories
      } catch (e) {
        this.isError = true
      } finally {
