<template>
  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="itemForm.marital_status"
                    required
                  />
                </div>
              </div>
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="selectedItem.marital_status"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="maritalStatus in orderedMainList"
        :key="maritalStatus.id"
        @dblclick.stop="showUpdateMainItemModalInList(maritalStatus.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="maritalStatus.isSelected"
            />
          </div>
        </td>
        ...
        <td>
          {{
            new Date(maritalStatus.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(maritalStatus.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(maritalStatus.id)"
        ...
        <div class="col-12">
          <div class="mb-3">
            ...
            <input
              ...
              v-model="searchForm.marital_status__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getMaritalStatusAPIInstance(),
      itemForm: Object.assign({}, getMaritalStatusAPIInstance().formData),
      searchForm: Object.assign({}, getMaritalStatusAPIInstance().searchObj),
      selectedItem: Object.assign({}, getMaritalStatusAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("maritalStatuses/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("maritalStatuses/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "maritalStatuses/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "maritalStatuses/getList",
    }),
  },
  watch: {
    searchForm: {
