<template>
  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    v-model="selectedItem.group_name"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="group in orderedMainList"
        :key="group.id"
        @dblclick.stop="showUpdateMainItemModalInList(group.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
        ...
        <td>
          {{
            new Date(group.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(group.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(group.id)"
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="group_name__icontains" class="form-label">Группа</label>
            <input
              type="text"
              class="form-control"
              id="group_name__icontains"
              v-model="searchForm.group_name__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getGroupAPIInstance(),
      itemForm: Object.assign({}, getGroupAPIInstance().formData),
      searchForm: Object.assign({}, getGroupAPIInstance().searchObj),
      selectedItem: Object.assign({}, getGroupAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("groups/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("groups/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch("groups/actionDeleteItem", this.deleteItemId)
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("groups/actionDeleteItem", item.id)
        }
      })
    ...
    debouncedSearch: debounce(async function () {
      try {
        await this.$store.dispatch("groups/actionGetList", this.searchForm)
      } catch (e) {
        this.isError = true
      } finally {
