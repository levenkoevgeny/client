<template>
  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="itemForm.military_office"
                    required
                  />
                </div>
              </div>
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="selectedItem.military_office"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="militaryOffice in orderedMainList"
        :key="militaryOffice.id"
        @dblclick.stop="showUpdateMainItemModalInList(militaryOffice.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="militaryOffice.isSelected"
            />
          </div>
        </td>
        <td>{{ militaryOffice.military_office }}</td>

        <td>
          {{
            new Date(militaryOffice.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(militaryOffice.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(militaryOffice.id)"
        ...
        <div class="col-12">
          <div class="mb-3">
            ...
            <input
              ...
              v-model="searchForm.military_office__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getMilitaryOfficeAPIInstance(),
      itemForm: Object.assign({}, getMilitaryOfficeAPIInstance().formData),
      searchForm: Object.assign({}, getMilitaryOfficeAPIInstance().searchObj),
      selectedItem: Object.assign({}, getMilitaryOfficeAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("militaryOffices/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("militaryOffices/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "militaryOffices/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "militaryOffices/getList",
    }),
  },
  watch: {
    searchForm: {
