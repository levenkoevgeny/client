<template>
  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="itemForm.foreign_language"
                    required
                  />
                </div>
              </div>
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    ...
                    type="text"
                    class="form-control"
                    v-model="selectedItem.foreign_language"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      <tr
        v-for="foreign_languages in orderedMainList"
        :key="foreign_languages.id"
        @dblclick.stop="showUpdateMainItemModalInList(foreign_languages.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
          >
            <input
              type="checkbox"
              class="form-check-input my-0"
              v-model="foreign_languages.isSelected"
            />
          </div>
        </td>
        <td>{{ foreign_languages.foreign_language }}</td>

        <td>
          {{
            new Date(foreign_languages.date_time_created).toLocaleString(
              "ru-RU",
              {
                day: "numeric",
                month: "long",
        ...
        <td>
          {{
            new Date(foreign_languages.date_time_updated).toLocaleString(
              "ru-RU",
              {
                day: "numeric",
                month: "long",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(foreign_languages.id)"
        ...
        <div class="col-12">
          <div class="mb-3">
            ...
            <input
              ...
              v-model="searchForm.foreign_language__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getForeignLanguageAPIInstance(),
      itemForm: Object.assign({}, getForeignLanguageAPIInstance().formData),
      searchForm: Object.assign({}, getForeignLanguageAPIInstance().searchObj),
      selectedItem: Object.assign({}, getForeignLanguageAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("foreignLanguages/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("foreignLanguages/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "foreignLanguages/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "foreignLanguages/getList",
    }),
  },
  watch: {
    searchForm: {
