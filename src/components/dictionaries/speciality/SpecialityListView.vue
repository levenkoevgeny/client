  ...
  >
    ...
    <template v-slot:tbody>
      ...
      >
        ...
        <td>{{ speciality.speciality_short_name }}</td>
        <td>{{ speciality.speciality_code }}</td>
        <td>
          {{
            new Date(speciality.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(speciality.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="speciality_name__icontains" class="form-label"
              >Специальность</label
            >
            <input
              type="text"
              class="form-control"
              id="speciality_name__icontains"
              v-model="searchForm.speciality_name__icontains"
            />
          </div>
        </div>
      </div>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="speciality_code__icontains" class="form-label"
              >Код специальности</label
            >
            <input
              type="text"
              class="form-control"
              id="speciality_code__icontains"
              v-model="searchForm.speciality_code__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getSpecialityAPIInstance(),
      itemForm: Object.assign({}, getSpecialityAPIInstance().formData),
      searchForm: Object.assign({}, getSpecialityAPIInstance().searchObj),
      selectedItem: Object.assign({}, getSpecialityAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("specialities/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("specialities/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteCheckedItemsHandler() {
      this.mainItemList.results.map(async (item) => {
        if (item.isSelected) {
          await this.$store.dispatch("specialities/actionDeleteItem", item.id)
        }
      })
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "specialities/getList",
    }),
  },
  watch: {
    searchForm: {
