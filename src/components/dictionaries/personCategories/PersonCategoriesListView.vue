<template>
  <base-list-layout
    ...
    title="Категории сотрудников, курсантов, студентов"
  >
    ...
    <template v-slot:tbody>
      <tr
        v-for="category in orderedMainList"
        :key="category.id"
        @dblclick.stop="showUpdateMainItemModalInList(category.id)"
      >
        <td>
          <div
            class="form-check d-flex align-items-center justify-content-center"
        ...
        <td>
          {{
            new Date(category.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(category.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          <div class="d-flex align-items-end justify-content-end">
            <button
              type="button"
              class="btn btn-outline-danger"
              @click="trashButtonClick(category.id)"
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="category__icontains" class="form-label"
              >Категория</label
            >
            <input
              type="text"
              class="form-control"
              id="category__icontains"
...
export default {
  ...
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getCadetCategoryAPIInstance(),
      itemForm: Object.assign({}, getCadetCategoryAPIInstance().formData),
      searchForm: Object.assign({}, getCadetCategoryAPIInstance().searchObj),
      selectedItem: Object.assign({}, getCadetCategoryAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("personCategories/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("personCategories/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "personCategories/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "personCategories/getList",
    }),
  },
  watch: {
    searchForm: {
