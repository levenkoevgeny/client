  ...
  >
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    v-model="itemForm.punishment_kind"
                    required
                  />
                </div>
              </div>
              ...
              <div class="modal-body">
                <div class="mb-3">
                  ...
                  <input
                    type="text"
                    class="form-control"
                    v-model="selectedItem.punishment_kind"
                    required
                  />
                </div>
              </div>
    ...
    <template v-slot:tbody>
      ...
      >
        ...
        <td>{{ punishment.punishment_kind }}</td>
        <td>
          {{
            new Date(punishment.date_time_created).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
        ...
        <td>
          {{
            new Date(punishment.date_time_updated).toLocaleString("ru-RU", {
              day: "numeric",
              month: "long",
              year: "numeric",
              hour: "numeric",
    ...
    <template v-slot:search-form>
      <div class="row">
        <div class="col-12">
          <div class="mb-3">
            <label for="gpunishment_kind__icontains" class="form-label"
              >Взыскание</label
            >
            <input
              type="text"
              class="form-control"
              id="punishment_kind__icontains"
              v-model="searchForm.punishment_kind__icontains"
            />
          </div>
        </div>
      </div>
...
export default {
  name: "PunishmentKindListView",
  components: { BaseListLayout },
  data() {
    return {
      isLoading: false,
      isError: false,
      mainItemAPIInstance: getPunishmentKindAPIInstance(),
      itemForm: Object.assign({}, getPunishmentKindAPIInstance().formData),
      searchForm: Object.assign({}, getPunishmentKindAPIInstance().searchObj),
      selectedItem: Object.assign({}, getPunishmentKindAPIInstance().formData),
      deleteItemId: "",
    }
  },
  methods: {
    ...
    async addNewItem() {
      try {
        await this.$store.dispatch("punishmentKinds/actionAddNewItem", {
          ...this.itemForm,
        })
      } catch (error) {
      } finally {
    ...
    async updateMainItemInList() {
      try {
        await this.$store.dispatch("punishmentKinds/actionUpdateItem", {
          ...this.selectedItem,
        })
      } catch (error) {
      } finally {
    ...
    async deleteItemHandler() {
      try {
        await this.$store.dispatch(
          "punishmentKinds/actionDeleteItem",
          this.deleteItemId,
        )
      } catch (error) {
      } finally {
  ...
  computed: {
    ...
    ...mapGetters({
      mainItemList: "punishmentKinds/getList",
    }),
  },
  watch: {
    searchForm: {
